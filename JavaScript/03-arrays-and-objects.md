...
average /= odds.length;
console.log(`odds's average is ${result}!`);

//自己写的
for (const [name, i] of Object.entries(game.odds)) {
  // console.log(typeof name);
  // console.log(name);
  //注意这个，用变量表达应该用另一种表达方式
  const name1 = game?.[name] ?? 'draw';
  let name2 = 'victory';
  if (!game?.name) name2 = '';
  console.log(`Odd of ${name2} ${name1} :${i}`);
}
